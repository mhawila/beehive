...
function prepareLocationInsert(rows, nextId) {
  ...
  rows.forEach(row => {
    ...
    let parentLocation = row['parent_location'] === null ? null : locationMap.get(row['parent_location']);

    locationMap.set(row['location_id'], nextId);
...
async function consolidateLocations(srcConn, destConn) {
  ...
  srcLocs.forEach(srcLoc => {
    ...
    if(match !== undefined && match !== null) {
      locationMap.set(srcLoc['location_id'], match['location_id']);
    }
    else {
